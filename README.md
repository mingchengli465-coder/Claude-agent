...
- 用客户自己的语言回答：写中文回中文，写英文回英文，写其他语言也跟着换；价格照
  资料原数字并注明人民币。语气像博主的助理。只根据 `products.yaml` 回答，写着「待填」
  的字段当作不知道。
- 固定的几句话（转人工、刷屏提示、收到文件）有中文和英文两版：客户写中文用中文，
  其他语言用英文。`/start` 欢迎语：Telegram 设成中文的只发中文，其他的中英都发
  （Telegram 没有官方中文界面，很多中文用户用的是英文界面，不能靠它判断）。
...
模型自己判断之外，还有一层关键词兜底（付款、下单、便宜点、真人、pay、discount、
real person……），命中就一定转。
...
- `/`：你的个人网站（`web_static/site.html`）：介绍 AI 客服搭建、Claude Code / Codex
  安装和其他服务、价格、常见问题、关于你。简体 / 繁體 / English 三种语言，默认中文
  （浏览器是繁体的显示繁體，其他都显示简体），右上角可以切换并会记住，聊天窗口跟着一起换。
- `/demo`：演示页，发给商户看效果；右下角就是真的聊天窗口。默认中文，`/demo?lang=en` 是英文版。
...
六项服务（写代码、做 PPT、写文案 / 小红书文案、做简单网站、帮商户搭建 AI 客服、帮你装好 Claude Code / Codex），每项有说明、价格区间、
